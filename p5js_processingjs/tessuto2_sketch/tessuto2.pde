...
void draw()
{
    ...
    //per ciascun mover, calcolo la forza delle molle che gli sono attaccate, lo muovo (quindi si aggiornano le molle di cui Ã�Â¨ chiodo)
    //e quindi cambieranno anche i movers associati alle molle di cui Ã�Â¨ chiodo
    for(MoverClass[] mover : movers)
    {
      for(MoverClass mover2 : mover)
      {
    ...
    pushMatrix();
        translate(width/2 - 160, height - 25);
        noStroke();
        fill(0,255,0);
