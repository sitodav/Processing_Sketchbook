...
void draw()
{
    ...
    if(mousePressed && mouseButton == LEFT && movers != null)
    {
       ...
       int numElemX = nMoversX / 15;
       int numElemY = nMoversY / 15;
       if(!(xMover < 1 || xMover > nMoversX-1 || yMover < 1 || yMover > nMoversY-1))
       {
         for(int i=-numElemX;i<=numElemX;i++)
         {
            ...
            {  
               ...
               movers[yMover+i][xMover+j].applyForce(f.mult(0.8));
            }
         }
       }
    }
